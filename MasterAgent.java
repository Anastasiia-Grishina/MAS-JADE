...
public class MasterAgent extends Agent {
	...
	public void CreateAgents(String agentsNumber,String host,String port,String tickerTime, String fibb) throws ControllerException {
        ...
        //Update GUI upon clicking create button
        if(armySize==0) {
        ...
        // Update GUI with the number of new agents
        agentGUI.logTA.append(agentsNumber+" soldiers joined attack\n");
        
        // Update internal agents counter
	...
	public void stopAttack() {
		...
		public void action() {
			...
			//Stop attack and kill agents, if agents exist
			if (0 != armySize) {
				...
				for (int i=1; i<=counter; i++) {
					ACLMessage msgACL = createMessage(
							ACLMessage.REQUEST, 
							"Stop", 
							new AID("SoldierAgent" + (counter-i), AID.ISLOCALNAME)
							);
					send(msgACL);
					armySize--;
				}

			// Update GUI upon stop button click
			agentGUI.logTA.setText("");
			agentGUI.logTA.append("Attack STOPPED!\n");
			agentGUI.existingAgentsNumberLabel.setText(""+0);
			} 
			else {
				//Repeatedly clicked on stop button
				armySize=0;
